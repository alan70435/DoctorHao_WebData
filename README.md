...
│   ├── posts/             #   23 則貼文全文（最新 20 則 + 歷史 3 則，粉專創立文追溯至 2023-03-15）
...
| Facebook 粉專 | 23 則全文 + 211 圖 | 2022-09（相簿最早）~ 2026-08 | ⚠️ 文字僅最新 20 則 + 散見歷史 3 則；照片相簿已全量 |
...
- **Facebook**：Googlebot UA 直抓個人頁（免登入取得最新 20 則全文），圖片經 `lookaside.fbsbx.com/lookaside/crawler/media/` 端點；歷史照片以 `photo.php?fbid=` 頁內 prev/next 相簿鏈 BFS 走完整本相簿（211 張，最早 2022-09），並以 owner 欄位過濾非本粉專照片；歷史貼文文字經 `permalink.php?story_fbid=` 單篇變體 + 新聞內嵌 share 連結追溯（+3 則，含粉專創立文）
- **Instagram**：格線縮圖 `ig_cache_key` 解碼換算 shortcode，`/p/<code>/embed/captioned/` 帶 Referer 取全解析度圖片

## 限制記錄

1. **Facebook 更舊貼文文字**：免登入 bot 視圖僅提供最新 20 則（`has_next_page:false`），歷史文字僅能靠外部連結追溯（目前 +3 則）。已確認死路：Common Crawl（robots.txt 全面排除 facebook.com）、Wayback Machine 與 archive.today（本頁零快照）、各大搜尋引擎（全被反爬阻擋）。若需完整貼文歷史，需粉專管理員提供 Graph API token（`/posts` 一次取回全部）
